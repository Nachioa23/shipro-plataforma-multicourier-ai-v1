...
## DEUDA 180 — Mercado Envíos Flex (MEF): integración con Mercado Libre (registrada 2026-09-13, lidera Chat D, **Fase 1 núcleo RESUELTA 2026-09-21 → ver DEUDAS-RESUELTAS.md**; pendiente conocido: RATIFICAR handshake con webhook REAL de ML — **sandbox de ML impracticable** (agotado 2026-09-17..2026-09-22: 4 ítems bloqueados por PolicyAgent + picture pending + web UI 2FA email inexistente); se ratifica con **primer webhook real** (venta productiva real o si el soporte ML destraba el sandbox); **receiver validado + aprobado por soporte ML como "estándar de oro"**. Aclaración: la prueba end-to-end de negocio (compra → envío → etiqueta) es **FASE 2** (routing/etiqueta, dirige Chat D), no Fase 1. **DEUDA nginx real-IP** registrada abajo como prerequisito de `ML_WEBHOOK_IP_ENFORCE=true`.)
