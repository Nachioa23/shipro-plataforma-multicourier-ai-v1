...
## DEUDA 150 — Centro de conexiones del cliente: UI para que las empresas administren sus plugins/apps/conectores y su API Key (registrada 2026-08-26, ampliada 2026-09-03, Pieza 1 modelo Conexion + flujo API Key Opción A DEPLOYADAS 2026-09-06, Pieza VISTA+REGISTRO manual DEPLOYADA 2026-09-07, Pieza 2 registro auto Tiendanube DEPLOYADA + VERIFICADA EN PROD 2026-09-07 commit `02cee46`, EN PROGRESO — Pieza 3 bloqueada por [[DEUDA 145]])

**Status:** EN PROGRESO — **4 piezas del hub en PROD** ✅: Pieza 1 (modelo Conexion) + Pieza API-Key (flujo seguro Opción A) + Pieza VISTA+REGISTRO (vista + registro manual en `/clientes`) + **Pieza 2 (registro automático de Conexion Tiendanube en el callback OAuth, VERIFICADA e2e 2026-09-07)**. Con Pieza 2 en prod, el hub deja de depender de registro manual para las tiendas Tiendanube — se auto-registran solas al instalarse. Pieza 3 (mandar plugin desde el hub) queda bloqueada por [[DEUDA 145]] (release del plugin de Chat C — el gate del `.zip` hosteado + versionado). Otros verbos del hub (mandar credenciales/docs, ciclo de vida suspender/reactivar, absorción total de Tiendanube) registrados abajo como follow-ups.
...
- **Pieza 3 — Mandar plugin desde el hub** — 🚧 **BLOQUEADA por [[DEUDA 145]]** (release del plugin WooCommerce por Chat C: el `.zip` hosteado en URL fija + versionado del gate). Diseño del lado Chat A confirmado: el operador clickea "Enviar plugin" en la conexión WooCommerce del cliente → mail con link de descarga del `.zip` + guía de instalación. Requiere el `.zip` hosteado (hoy vive solo en la Mac de Nacho como artefacto del repo `shipro-woocommerce`). Resolución acordada: Chat A hostea el `.zip` en un endpoint público del núcleo (ej. `/downloads/shipro-woocommerce-<version>.zip` servido desde `public/downloads/`) o en un bucket externo, **avisando a Chat C** que su repo `shipro-woocommerce` sigue siendo la fuente y el `.zip` se copia desde ahí en cada release. Reusa el mecanismo de mail existente (`lib/mailer.ts` — nuevo `enviarMailPlugin` moldeado sobre `enviarMailSetupApiKey`). No arranca hasta que Chat C libere el gate de [[DEUDA 145]].
