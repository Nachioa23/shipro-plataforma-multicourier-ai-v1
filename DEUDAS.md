...
**AVANCE 2 (2026-08-31, commit `0b1f6b0`):** SEGUNDO campo — `costoCourierEsperado → costoCourierCotizado`, vía `@map("costoCourierEsperado")` (columna física intacta, cero migración). **CRÍTICO manejado**: el snapshot JSON de `ConciliacionRun` (undo path) conserva la key vieja `"costoCourierEsperado"` como **bridge** → deshacer conciliaciones viejas sigue funcionando (round-trip: `WRITE snapshot { costoCourierEsperado: finanzas.costoCourierCotizado } ↔ READ snapshot restore Prisma.costoCourierCotizado ← entry.prior.costoCourierEsperado`). Par cotizado/facturado coherente ahora (`costoCourierCotizado` + `costoCourierFacturado`). Verificado `tsc=0` + `prisma migrate status` up-to-date + bridge round-trip. Blast radius: **3 files** (schema + `conciliacion/route.ts` + `conciliacion/revertir/route.ts`) — mucho más chico que el rename #1 porque el field es interno-admin (zero frontend, zero raw SQL, zero métricas). Pendiente deploy prod.
