...
- ⏳ **`FinanzasEnvio.tarifaCourierBase → tarifaCourierBaseNeta`** — hoy persiste NETIZADA (post-IVA-strip); el sufijo `Neta` desambigua "cruda" del comment. **Post-corrección-de-criterio (2026-08-31):** se renombra a pesar de que el nombre "Base" es defensible — DEUDA 158 va por TODOS los campos, no solo los "muy engañosos".
- ⏳ **`FinanzasEnvio.markupIntermediarioAplicado → markupIntermediarioPorcentajeAplicado`** — persiste solo el `+%` del intermediario (`baseConIntermediario − secoNeto`); el `+fijo` es ghost. **Nacho confirmó (2026-08-31)** que el fijo intermediario NO va a existir nunca → nombre "solo porcentaje" es DEFINITIVO. Feed colateral a [[DEUDA 160]] (`seguroFijoIntermediarioConIva` ahora ghost firme).
...
**Progress: 5/N. Restantes: 5 fields (3 standalone + 2 entangled con DEUDA 157).**
