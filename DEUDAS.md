...
## DEUDA 180 — Mercado Envíos Flex (MEF): integración con Mercado Libre (registrada 2026-09-13, lidera Chat D, scope grande, **Fase 1 steps 1+2+3 HECHOS local 2026-09-14** commits `b53426d`+`18d33c3`+`6ea745b`+`cbc96da`; steps 4-5 pendientes; único follow-up abierto de Fase 1 = mail de alerta cross-install; ningún deploy prod aún, gated manual)
...
- **⚠️ Follow-up abierto de Fase 1 (no bloqueante): `enviarMailAlertaCruceMercadoLibre` helper en `lib/mailer.ts`.** El cross-install guard hoy hace `AuditoriaConfiguracion.create` + `console.error` — falta el mail al equipo Shipro (mirror del twin `enviarMailAlertaCruceTiendanube` en `lib/mailer.ts:667`). El caso raro (cross-install) ya queda capturado en `AuditoriaConfiguracion` — el mail es solo visibilidad extra. Se agrega cuando se toque `lib/mailer.ts` (aditivo, ~40 líneas). TODO documentado en el callback.

**Steps 4-5 (webhook receiver + adapter)** — territorio a coordinar con Chat D, después del step 3.

**Estado global**: **Fase 1 completa localmente** (steps 1+2+3 hechos, andamiaje operativo). Steps 4-5 por delante bajo dirección de Chat D. **Ningún deploy a prod de MEF todavía** — sigue gated manual por Nacho + Chat D. Env vars `MERCADOLIBRE_CLIENT_ID` + `MERCADOLIBRE_CLIENT_SECRET` pendientes (local para probar el flujo end-to-end en dev; prod cuando se abra el gate).
