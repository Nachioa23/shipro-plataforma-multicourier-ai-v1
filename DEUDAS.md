...
## DEUDA 180 — Mercado Envíos Flex (MEF): integración con Mercado Libre (registrada 2026-09-13, lidera Chat D, scope grande, **Fase 1 (núcleo) EN PROD 2026-09-15** — steps 1+2+3+4 deployados + verificados, commits `b53426d`+`18d33c3`+`6ea745b`+`cbc96da`+`cf49e93`+`e0634e7`; prueba OAuth e2e + fases 2-5 las dirige Chat D)
...
- **Prueba OAuth e2e**: conectar una cuenta ML real desde el hub → verificar que se guarde en `CuentaMercadoLibre` con tokens encriptados (formato `<hex>:<hex>:<hex>`), `estado="activa"`, `mlUserId` real, `nickname` (best-effort). Requiere: (a) el método de generar el install-link — hoy sin UI, se dispara via DevTools Console del browser con un `fetch("/api/mercadolibre/install/link", {method:"POST", body: JSON.stringify({empresaId})})`; alternativa: agregar un botón "Conectar ML" al hub `/clientes` (~30 líneas mirror del `enviar-link-api-key`, additive, se hace cuando Chat D lo pida). (b) que el portal de developers de ML tenga registrada la Redirect URI de prod exacta `https://pm.shipro.pro/api/mercadolibre/oauth/callback` (byte-a-byte). Sin (b), ML rechaza el consent con `redirect_uri mismatch`.
- **Webhook e2e**: queda para cuando haya un envío Flex real (el receiver ya está en prod escuchando + dedup idempotente por `notificacionId @unique`).
