...
**Status:** ABIERTA — **6 de N campos renombrados**: (i) `precioFactura → tarifaFullCotizada` (commit `a8cda53` 2026-08-30), (ii) `costoCourierEsperado → costoCourierCotizado` (commit `0b1f6b0` 2026-08-31), (iii) `seguroAplicado → smoAplicado` (commit `26855dc` 2026-08-31), (iv-v par) `precioProveedor → costoCourierNativo` + `precioProveedorReal → baseConIntermediarioAplicado` (commit `09f3ca4` 2026-08-31, con bridge JSON snapshot `CotizacionSnapshot`), (vi) `tarifaCourierBase → tarifaCourierBaseNeta` (commit `f2ae054` 2026-08-31). Los comments honestos de FASE 1 mitigan el riesgo de lectura para los restantes, pero el nombre engañoso sigue induciendo bugs (ver [[DEUDA 156]] fallout: 4 readers usaron precioMostrado como billed proxy hasta que la semántica se alineó). Se hace de a UN campo por sesión (o de a UN PAR cuando los nombres viejos comparten ambigüedad, como el par proveedor/proveedorReal) — principio Nacho: money-safe, nunca en lote.
...
**Progress: 6/N. Restantes: 4 fields (2 standalone + 2 entangled con DEUDA 157).**
