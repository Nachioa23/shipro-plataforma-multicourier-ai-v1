...
- **Pieza "Registrar + ver conexiones"** — ✅ **HECHA LOCAL 2026-09-07 (commit `a7e128f`), pendiente deploy**. Poblar el modelo `Conexion` + pantalla del hub para ver quién está integrado a qué. Detalle abajo en "Avance 2026-09-07 — Pieza vista + registro de conexiones". Habilita el instructivo por plataforma + la vista consolidada del cliente.
...
**Piezas siguientes del hub (coordinadas, requieren negociación cross-chat):**

- **Pieza 2 — Poblar Conexion desde Tiendanube (auto).** Agregar upsert de `Conexion(plataforma=TIENDANUBE, mecanismo=OAUTH, referenciaExterna=<storeId>)` en el callback OAuth de Tiendanube (`app/api/tiendanube/oauth/callback/route.ts:118`, junto al `upsert(tiendaTiendanube)`). El cambio es aditivo (una segunda mutación Prisma en la misma tx), pero **toca territorio Chat B** → requiere aviso/autorización a Chat B antes de tocar. Con esto, el hub muestra las tiendas Tiendanube como Conexion rows nativas sin lectura dual (`TiendaTiendanube` sigue siendo fuente de verdad histórica; `Conexion` es el índice del hub). Diferente de la "absorción completa" (migrar tabla), que sigue siendo tarea posterior.
- **Pieza 3 — Mandar plugin desde el hub.** El operador clickea "Enviar plugin" en la conexión WooCommerce del cliente → mail con link de descarga del `.zip` + guía de instalación. Requiere el `.zip` **hosteado en URL fija** (hoy vive solo en la Mac de Nacho como artefacto del repo `shipro-woocommerce`). Resolución: Chat A hostea el `.zip` en un endpoint público del núcleo (ej. `/downloads/shipro-woocommerce-<version>.zip` servido desde `public/downloads/`) o en un bucket externo, **avisando a Chat C** que su repo `shipro-woocommerce` sigue siendo la fuente y el `.zip` se copia desde ahí en cada release. Reusa el mecanismo de mail existente (`lib/mailer.ts` — nuevo `enviarMailPlugin` moldeado sobre `enviarMailSetupApiKey`).
- **Otros verbos**: mandar credenciales de courier / mandar docs por plataforma / ciclo de vida (suspender/reactivar/dar-de-baja usando `EstadoConexion`) / absorción completa de Tiendanube al modelo Conexion (Cross-chat con B). Cada uno es una pieza chica separada.
