...
## DEUDA 150 — Centro de conexiones del cliente: UI para que las empresas administren sus plugins/apps/conectores y su API Key (registrada 2026-08-26, ampliada 2026-09-03, Pieza 1 modelo Conexion + flujo API Key Opción A DEPLOYADAS 2026-09-06, Pieza VISTA+REGISTRO manual DEPLOYADA 2026-09-07, Pieza 2 registro auto Tiendanube DEPLOYADA + VERIFICADA EN PROD 2026-09-07 commit `02cee46`, **Pieza 3 mandar plugin ✅ COMPLETA + EN PROD 2026-09-11 commits `eb0873b`+`b1fa887`**, verbos adicionales pendientes como follow-ups)

**Status:** EN PROGRESO — **5 piezas del hub en PROD** ✅: Pieza 1 (modelo Conexion) + Pieza API-Key (flujo seguro Opción A) + Pieza VISTA+REGISTRO (vista + registro manual en `/clientes`) + Pieza 2 (registro automático de Conexion Tiendanube en el callback OAuth, VERIFICADA e2e 2026-09-07) + **Pieza 3 (mandar plugin desde el hub, mail con `.zip` del release + guía PDF, VERIFICADA e2e 2026-09-11)**. Con las 5 piezas vivas, el hub ya soporta el ciclo completo de onboarding self-service para WooCommerce (el operador registra la conexión → manda link para API Key → manda plugin) y auto-registro para Tiendanube. Verbos del hub aún pendientes como follow-ups no urgentes: mandar credenciales de courier, mandar docs por plataforma, ciclo de vida (suspender/reactivar/dar-de-baja usando `EstadoConexion`), absorción total de Tiendanube al modelo Conexion (Cross-chat con B).
...
- **Pieza "Registrar + ver conexiones"** — ✅ **DEPLOYADA EN PROD 2026-09-07 (commit `a7e128f`)**. Poblar el modelo `Conexion` + pantalla del hub para ver quién está integrado a qué. Detalle abajo en "Avance 2026-09-07 — Pieza vista + registro de conexiones". Habilita el instructivo por plataforma + la vista consolidada del cliente. Verificación de que está viva en prod: la vista renderizó correctamente la `Conexion(TIENDANUBE, ACTIVA)` recién creada por Pieza 2 al reinstalar la app demo (2026-09-07).
...
**Avance 2026-09-07 — Pieza vista + registro de conexiones (commit `a7e128f`, HECHA local, pend. deploy).** Primer verbo funcional del hub. 3 archivos, +401/-4:

- **Vista** (sub-sección "Conexiones del cliente" en el drawer per-cliente de `/clientes`): muestra las `Conexion` rows del cliente + tiendas Tiendanube (read-only, sin migrar — la absorción sigue pendiente como pieza posterior coordinada con Chat B) + estado de la API Key (activa/últimos4/creadaEn). `useEffect` dispara al abrir el drawer; loading + error states manejados; cierre del drawer limpia el estado.
- **Registro manual** (Opción B lockeada con Nacho): form de 3 selects (plataforma / mecanismo / estado) + botón "Registrar / Actualizar conexión" que POSTea al endpoint admin nuevo `POST /api/admin/empresas/[id]/conexiones` con **upsert** sobre `@@unique([empresaId, plataforma])` — respeta el constraint sin romper con duplicados; registrar la misma plataforma actualiza mecanismo/estado. Whitelist server-side con los 3 enums Prisma (`PlataformaConexion`, `MecanismoConexion`, `EstadoConexion`). Gate `admin_shipro/operador_shipro` (mismo patrón que `/enviar-link-api-key`).
- **Población automática al generar API Key**: dentro del `$transaction` existente del `POST /api/empresa/api-key/via-token`, tras persistir la key en `Empresa.apiKey*`, se hace upsert de `Conexion(mecanismo=API_KEY)`. Si Shipro ya registró manualmente una `Conexion(API_KEY)` para ese cliente antes → estado pasa a `ACTIVA`. Si no hay ninguna → se crea una placeholder `plataforma=API_REST, mecanismo=API_KEY, estado=ACTIVA` (el operador puede refinar la plataforma después). Atomic — si el upsert de Conexion falla, la generación de key también rollback.
- **DECISIÓN de producto registrada (Opción B, Nacho, 2026-09-07)**: la plataforma de un cliente API_KEY se registra **manualmente por Shipro** en el hub. Fundamento: la API Key es platform-agnóstica (una empresa la usa para todos sus plugins API-based); hoy NO existe ninguna señal en las requests que identifique la plataforma (grep del proxy + WooCommerce plugin confirmó cero `X-Shipro-Platform` header, cero identificación de plataforma en el body). Alternativas descartadas por ahora: (a) header `X-Shipro-Platform` en cada plugin → requiere Chat C tocar todos los plugins; (c) plugin self-registra en bootstrap → requiere Chat C también. La Opción B es la única que no depende de Chat C y funciona hoy. Auto-registro híbrido (a+b) queda como mejora futura una vez que Chat C esté disponible.
- **Chat B UNTOUCHED**: cero cambios en `app/api/tiendanube/*`, `lib/tiendanube/*`, `lib/couriers/*`. Las tiendas Tiendanube son solo LEÍDAS (via `prisma.tiendaTiendanube.findMany` en el endpoint admin). Sin schema change, sin migración. tsc = 0.

**Piezas siguientes del hub (estado actualizado 2026-09-07):**

- **Pieza 2 — Poblar Conexion desde Tiendanube (auto)** — ✅ **DEPLOYADA + VERIFICADA EN PROD 2026-09-07 (commit `02cee46`)**. Detalle abajo en "Avance 2026-09-07 — Pieza 2 verificada en prod". Autorizada + revisada por Chat B (3 condiciones cumplidas: post-transacción, best-effort, aditivo). Con Pieza 2 viva, el hub muestra las tiendas Tiendanube como Conexion rows nativas sin lectura dual (`TiendaTiendanube` sigue siendo fuente de verdad histórica; `Conexion` es el índice del hub). Diferente de la "absorción completa" (migrar tabla), que sigue siendo tarea posterior.
- **Pieza 3 — Mandar plugin desde el hub** — ✅ **RESUELTA + EN PROD 2026-09-11 (commits `eb0873b` feat + `b1fa887` fix del texto de la API Key).** El operador, desde el drawer per-cliente de `/clientes` sobre una conexión WooCommerce, clickea "Enviar plugin al cliente" → mail al gerente_cliente con: (i) link del `.zip` del release v0.1.0 del plugin (URL fija anclada a versión: `https://github.com/Nachioa23/shipro-woocommerce/releases/download/v0.1.0/shipro-woocommerce-0.1.0.zip`, bumpear versión = update explícito de la constante), (ii) link a la guía PDF (`https://nachioa23.github.io/shipro-api-docs/guia-instalacion-woocommerce.pdf`, hosteada por Chat C en el repo `shipro-api-docs` GitHub Pages), (iii) texto correcto de la API Key: **llega por separado vía link seguro (7 días, un solo uso), NO se pide del panel** — alineado con el flujo real del hub (Pieza API-Key). 4 archivos (+248/-12):
  - `lib/mailer.ts` — nueva función `enviarMailPlugin(email, nombre, urlZip, urlGuia)` moldeada sobre `enviarMailSetupApiKey` (mismo `transporter` + `firmaShipro`); rama de degradación: si `urlGuia` viene vacío, el mail se manda igual con el `.zip` + caja azul "Guía paso a paso: te la vamos a mandar por separado en las próximas horas" (no bloquea el envío).
  - `app/api/admin/empresas/[id]/enviar-plugin/route.ts` — endpoint nuevo POST, gate `admin_shipro/operador_shipro` (mismo patrón que `enviar-link-api-key`); resuelve destinatario como el `gerente_cliente` activo más antiguo; sin token (link público al release, idempotente); respuesta `{ok, empresaId, enviadoA, incluyeGuia}`.
  - `lib/constants/plugin.ts` — nuevo: `ZIP_PLUGIN_WOOCOMMERCE_URL` fija a v0.1.0 + `GUIA_PLUGIN_WOOCOMMERCE_URL = process.env.GUIA_PLUGIN_WOOCOMMERCE_URL ?? ""` (env var completable con default degradado).
  - `app/(dashboard)/clientes/page.tsx` — botón "Enviar plugin al cliente" en la fila de cada `Conexion`, **WooCommerce-gated** (`{c.plataforma === "WOOCOMMERCE" && ...}`) — no aparece en TIENDANUBE ni futuras plataformas; state per-conexión (`enviandoPluginId` + `mensajePlugin` indexado por `c.id`); mensaje "Plugin enviado a … (con guía)" o "(guía por separado)" según `incluyeGuia`.
  - Env var en prod: `GUIA_PLUGIN_WOOCOMMERCE_URL=https://nachioa23.github.io/shipro-api-docs/guia-instalacion-woocommerce.pdf` (agregada al `.env` de pm.shipro.pro, línea 71).
  - **Verificación e2e en prod (2026-09-11)**: se registró una `Conexion(WOOCOMMERCE)` en un cliente de test → el botón "Enviar plugin al cliente" apareció **solo** en esa fila (no en las TIENDANUBE del mismo cliente) → click → mail llegó completo con el `.zip` + link a la guía PDF + texto correcto de la API Key.
  - **Coordinación Chat C CERRADA**: el PDF ya vive en `shipro-api-docs` GitHub Pages, URL definitiva; el **Paso 1** del PDF dice exactamente lo mismo que el mail (la API Key llega por separado vía link seguro), evitando la contradicción que traía el borrador previo (que decía "pedila del panel"). Mail + guía cuentan la misma historia end-to-end.
  - **Deploy paquete 2026-09-11 (2eb723f..b1fa887, 9 commits)** trajo también: la limpieza del menú de la Consola de Tarifa (`281e4cd` — 4 pantallas viejas fuera del menú, [[DEUDA 170]] cerrada del todo) + el fix del texto de la API Key en el mail (`b1fa887`) + Reintento P2+P3 código (`c704a13`+`7eeaefc` — endpoints `/api/cron/reintentar-despachos` + `reintentarUnEnvio` race-safe). Sin migración, sin schema drift, sin cambios de money paths. Todo verificado.
- **Otros verbos**: mandar credenciales de courier / mandar docs por plataforma / ciclo de vida (suspender/reactivar/dar-de-baja usando `EstadoConexion`) / absorción completa de Tiendanube al modelo Conexion (Cross-chat con B, hoy `Conexion(TIENDANUBE)` coexiste con `TiendaTiendanube` — la absorción total es follow-up mayor). Cada uno es una pieza chica separada.

**Avance 2026-09-07 — Pieza 2 verificada en prod (commit `02cee46`).** Best-effort upsert en el callback OAuth de Tiendanube: registra `Conexion(plataforma=TIENDANUBE, mecanismo=OAUTH, estado=ACTIVA, referenciaExterna=<storeId>)` automáticamente al terminar el install del cliente. Aditivo puro (+32/-0 en `app/api/tiendanube/oauth/callback/route.ts`) — cero schema, cero migración, cero cambio al store upsert existente.

- **Diseño del safety** (verificado por Chat B, 3 condiciones): (i) va **POST-COMMIT** de la `$transaction([tiendaTiendanube.upsert, tokenVinculacionTiendanube.update])` — la instalación del store ya cerró; (ii) envuelto en `try/catch` con `console.error` sin re-lanzar → si el upsert del hub falla, la instalación de la tienda **NO se rompe** (la venta/install nunca se pierde); (iii) es aditivo (ninguna otra línea del callback se toca, ningún schema/migration).
- **Verificación e2e en prod (2026-09-07)**: reinstalación de la app demo Tiendanube → `Conexion(empresaId=<demo>, plataforma=TIENDANUBE, mecanismo=OAUTH, estado=ACTIVA, referenciaExterna=<storeId>)` apareció en el drawer per-cliente de `/clientes` sin registro manual → nueva venta en la tienda demo → **etiqueta `360003093978780` generada correctamente** → circuito Tiendanube (rates + install + venta + etiqueta) **intacto**, ninguna regresión.
- **Multi-store caveat conocido**: el `@@unique([empresaId, plataforma])` implica que si una empresa tiene una 2da tienda Tiendanube, el upsert pisa `referenciaExterna` (last-store-wins). Aceptable por ahora — el detalle real de N tiendas ya vive en `TiendaTiendanube` y el hub lo expone read-only aparte via `app/api/admin/empresas/[id]/conexiones` GET, que trae `tiendasTiendanube` listadas. Si el día de mañana se necesita reflejar N Tiendanube-Conexion-rows por empresa, se relaja el `@@unique` (documentado en el comment del schema `Conexion`).
- **Mapeo estado (documentado en el código)**: la callback OAuth solo entra por el camino feliz del install → `Conexion` arranca `ACTIVA`. Los estados `"suspendida"→PENDIENTE` y `"desinstalada"→REVOCADA` los aplicarán los webhooks lifecycle (`app/suspended`, `app/resumed`, `app/uninstalled`) en su propia pasada — fuera de scope de Pieza 2, forma parte del follow-up "ciclo de vida" listado arriba.
...
- **MEJORA A — Preview fiel a la credencial real** — ✅ **HECHA + VERIFICADA local 2026-09-13 (commit `f697e52`, 1 archivo `app/api/admin/consola-tarifa/preview/route.ts` +74/-12).** El endpoint del preview ahora consulta `prisma.credencialCourier.findUnique({ where: { empresaId_nombreCourier: { empresaId, nombreCourier: courier.nombre } } })` (mismo lookup canónico que `crear.ts:396-398`, usa el `@@unique([empresaId, nombreCourier])`) y usa los valores reales (`usaCredencialesPropias`, `propietarioTipo`) para armar el `ConfigMarkup`. Los toggles del body pasan a ser **overrides opcionales** — si vienen se usan (modo exploratorio), si no vienen se usan los reales. Response aditivo: nuevo bloque `credencial: { real, sinCredencial, credencialInactiva, usandoOverride }` sin renombrar/mover ningún campo existente. **Verificación por caso** (todos vía preview live sobre la BD real): (a) Rama A + COURIER → mismos números que el default previo (comportamiento byte-idéntico si esa era la config real); (b) Rama B → cascada sin markups (chain = tarifa + Fee, sin markup Shipro ni intermediario); (c) `propietarioTipo=SHIPRO` → sin markup intermediario; (d) empresa sin credencial → `sinCredencial:true` + fallback neutral Rama A/COURIER (preview computa igual, no se oculta — la consola es exploratoria); (e) override por body → toggles pisan al real. **Display-only**: cero cambios en `aplicarMarkup`, en los 4 resolvers, en `crear.ts`/`corregir`/`cancelar` ni en ningún money path. Endpoint sigue **READ-ONLY** (1 `findUnique` nuevo, cero writes). Ni schema ni migración. tsc = 0. **La sub-pieza de UI (pre-seed de los toggles avanzados con el valor real + nota `sinCredencial`) NO hizo falta como bloqueante** — el endpoint alcanzó por sí solo (los toggles siguen funcionando como override para exploración). La nota de UI queda como pequeño follow-up opcional junto con Mejora B.
...
**Resolución de courier normalizada (COMMIT `caf158b`, LOCAL, pendiente deploy, 2026-09-06).** Segunda mitad del fix — la que hace que Chat C no necesite mantener un mapeo de nombres. `crear.ts` resolvía el courier vía `obtenerCourier` que usa el normalizer viejo (`normalizarParaComparacion`: lowercase + strip apóstrofes/espacios, sin NFD): funcionaba para `"ANDREANI"`/`"MOCI'S"`/`"INTRALOG"` pero fallaba para variantes sin acento tipo `"HOP ENVIOS"` (vs BD `"Hop Envíos"`). **FIX**: reemplaza el helper viejo por el nuevo `normalizarNombreCourier` (`lib/utils/normalizar-courier.ts`, del fix de Hop [[DEUDA 164]]) que agrega NFD + strip diacríticos. Inline en `crear.ts`: `findMany` + `.find(c => normalizarNombreCourier(c.nombre) === normalizarNombreCourier(nombreCourier))`. Ahora acepta **cualquier variante de case + acento + apóstrofe + espacio** del nombre canónico. Guard `CourierAusente` queda como fallback SOLO si ningún courier normalizado coincide (ej. `"FEDEX"` → sí sigue tirando 400). tsc = 0, solo `crear.ts` (+37/-19 netos, incluye remoción del import residual `obtenerCourier` ya no usado en este archivo — el helper sigue vivo en `lib/couriers/normalizar.ts` para otros callers).

**Contrato canónico para todos los plugins (WooCommerce y futuros VTEX/Shopify/etc.):** `POST /api/envios` espera el courier en el campo **`nombreCourier`** (camelCase). El valor es el string **`courier` que devuelve `/api/cotizar` TAL CUAL** (`ANDREANI` / `MOCI'S` / `INTRALOG` / `HOP ENVÍOS` / `CORREO ARGENTINO` / `OCA`). **El servidor normaliza** — ningún plugin mantiene un mapeo. Failure mode: si el string no matchea ningún canónico BD normalizado → 400 con code `COURIER_AUSENTE`. **Regla operacional**: cualquier plugin nuevo que se integre a `/api/envios` debe copiar `OpcionTarifa.courier` como-viene al request de creación de envío; no transformar, no re-normalizar, no mapear.

**Estado del bug WooCommerce-etiqueta:** Fix A (guard 500→400) ✓ local. Resolución normalizada ✓ local. **Falta**: Chat C manda `nombreCourier` desde el plugin (build + deploy del plugin) → validación e2e (compra WooCommerce demo → etiqueta real de Andreani/Moci's/Intralog generada). Ambos commits van al próximo deploy prod. Fix B (degradación "venta nunca se pierde" en vez de `CourierAusente` 400) sigue **PENDIENTE** como obra de robustez — separado de esta línea.
...
**Ampliación 2026-09-06 — Normalización de dirección en el núcleo para el camino de plugins.**

Contexto: WooCommerce YA funciona e2e (etiqueta Andreani `360003092333370` validada), pero el primer envío exitoso nació RETENIDO y requirió corrección manual del comprador porque el plugin manda datos con formato distinto al que espera Andreani (provincia como código ISO "B", calle+altura juntas). Estos 2 fixes automatizan la corrección — el envío nace CREADO sin intervención humana. **Chat B (adapters Andreani/Intralog) UNTOUCHED** en ambos — los normalizers viven en núcleo y aplican al armado del payload ANTES del adapter.

- **Fix 1 — Provincia desde CP (commit `df3537a`).** El plugin mandaba `provinciaDestino` = `shipping_state` = código ISO ("B" = Buenos Aires), que llegaba literal al adapter y salía en la etiqueta física sin sentido logístico. FIX: derivar provincia (+ localidades) del `cpDestino` usando la BD oficial (`CodigoPostal`/`Localidad`/`Provincia`, cargada desde `prisma/data/codigos.csv` via seed). La lógica autoritativa vivía inline en `app/api/geografia/buscar/route.ts` (endpoint que consumen `/nuevo-envio`, `/corregir`, `/auditoria`) — se **EXTRAJO a helper compartido `lib/geo/resolver-cp.ts`** para reuso desde `crear.ts` sin duplicar código (evita el patrón Mocis de doble fuente de verdad). El endpoint se refactorizó a thin-wrapper HTTP sobre el helper (zero API surface change; consumers actuales no notan). Resuelve además la ambigüedad de CABA automáticamente (la BD tiene `"CIUDAD AUTONOMA DE BUENOS AIRES"` → `normalizarProvincia` lo canonicaliza a `"CABA"`, mismo output que `/nuevo-envio` ya usaba). **Fallback**: si el CP no está en la BD (2.183 CPs cargados de ~5.000 argentinos) → `provinciaDestino = ""` → `validar-direccion` (Google Maps) dispara RETENIDO para corrección manual. NO adivinar. El commit descartó también la tabla código→nombre `lib/utils/provincias-ar.ts` que se había intentado en `23c3f72` (approach superado por el CP-based; el archivo se **eliminó** — cero consumers residuales).
- **Fix 2 — Parser de altura desde calle (commit `25e42ef`).** El plugin manda `calle` con la calle completa incluyendo el número ("Rector Eufemio Uballes 6802"); WooCommerce por default no tiene un campo separado para altura. Andreani (y la mayoría de adapters) necesitan `altura` separada. FIX: helper `lib/utils/parse-direccion.ts` con `parseAltura(calle)` **conservador** — extrae el número solo cuando puede identificarlo con confianza. Reglas: (1) detecta "S/N"/"sin número" → null; (2) detecta "Km"/"kilómetro" → null (dirección rural); (3) strippea suffix "Piso X"/"Depto Y"/"PB"/"Casa"/"Torre"/etc. antes de extraer; (4) trailing number → altura; (5) bias contra nombres numéricos ("Calle 50", "Boulevard 25" → null si el calle limpio queda en 1 token genérico). Verificado en vivo contra 11 casos. Aplicado en `crear.ts` SOLO cuando el plugin mandó altura vacía Y calle con contenido (respeta altura explícita). null → altura vacía → `validar-direccion.ts:71-74` dispara RETENIDO (si además calle no tiene keyword tolerada tipo "km"/"s/n"). Bias explícito: **falsos positivos (RETENIDO por rechazo) son aceptables — el comprador corrige via link; falsos negativos (etiqueta con altura errónea) son PEOR — el paquete no llega**.

**Contrato ampliado para todos los plugins** (WooCommerce, VTEX, Shopify, Mercado Libre futuros): el plugin manda `cpDestino` + `calle` completa (equivalente a WC `address_1`) + `provinciaDestino` tal cual del e-commerce (código o nombre, no importa). **El núcleo deriva provincia del CP y parsea altura de calle** — ningún plugin mantiene tablas AR ni heurísticas de parseo. Consistente con el contrato ya establecido para `nombreCourier` (server normaliza, plugins mandan `courier` de `/cotizar` tal cual).
...
- **Pieza 3 — Rama A: NO debitar hasta etiqueta courier real. HECHA + VERIFICADA local 2026-09-10 (commit `9ac92d9`, 2 archivos: `crear.ts` + `corregir/route.ts`).** Guard L1116 refactorizado en 2 condiciones semánticas: `hayEtiquetaCourierReal` (ningún flag de bloqueo/retención — incluye ahora `!falloPorPeaje && !bloqueadoPorDatosPaquete`) + `debeDebitar = esRamaB ? !bloqueadoPorSaldo : hayEtiquetaCourierReal`. Rama A ahora NO debita al crear en RETENIDO/DATOS_PAQUETE (fix bugs #1 #2). En `/corregir` RAMA 1 (dispatch OK genera etiqueta courier) se agrega DEBITO_ENVIO diferido usando helper delta (Rama A: yaAplicado=0 → cobra chain; Rama B: yaAplicado=Fee → delta=0 skip; retry idempotente). **Verificación e2e local**: crear Rama A + RETENIDO → 0 débito; corregir OK → DEBITO_ENVIO full chain aparece; camino feliz sin bloqueo → idéntico a pre-P3; Rama B en todos los estados → byte-idéntica a P2. Rama A never-labelled queda en 0 charge (gap intencional, se cierra en P4).
- **Pieza 4 — Cancelación rama-aware. HECHA + VERIFICADA local 2026-09-10 (commit `2e1d745`, 4 archivos: `cancelar/route.ts` + `tiendanube/labels/cancel/route.ts` + 2 helpers nuevos).** Nuevos helpers: `lib/finanzas/credito-aplicado.ts` (analog del debit-helper P1: suma CREDITO_* por tipo explícito) + `lib/finanzas/cancelacion-financiera.ts` (política financiera compartida entre los 2 endpoints de cancel). Rama A + chain debitado → reembolso `logisticaNetaFacturada × IVA_AR_MULTIPLIER` (misma fórmula que `cron/sweep-6m`) vía **nuevo tipo `CREDITO_CANCELACION`** + marca `finanzas.logisticaDevuelta=true` + `fechaDevolucionLogistica=now` (excluye sweep-6m auto). Rama A + never labelled → cobra Fee que nunca se cobró (cierra gap P3). Rama B → nada (Fee ya cobrado P2). **Decisión crítica de diseño**: signal "chain fue debitado" usa el **LEDGER** (`debitoAplicadoEnvio > feeConIva`), NO `logisticaNetaFacturada > 0` — este último se persiste al alta con el desglose del motor INDEPENDIENTEMENTE del gate de débito y daría falso positivo (Rama A + RETENIDO tiene `logisticaNetaFacturada=11766.55` pero yaDebitado=0). El breakdown se usa solo para calcular el **monto** del refund, no como decisión. Anti-doble-crédito via `creditoAplicadoEnvio(envioId, tx, ["CREDITO_CANCELACION"])` + guard `logisticaDevuelta`. **Verificación empírica**: envío `SHP-620556` y `SHP-376020` (Rama A + RETENIDO cancelados post-P3+P4) — ambos con `logisticaNetaFacturada=11766.55` persistido pero 0 debitado al crear → cancel emite 1 sola fila `DEBITO_ENVIO -1936` (Fee), 0 refund. Neto cobrado = $1.936 (solo Fee). ✅ política Nacho. Doble-cancel imposible por guard L33 + helpers. `MovimientoFinanciero.tipo` es String libre en schema → cero migración. Cero touch a P2-P3 (crear.ts + corregir intactos).
- **Pieza 5 — Courier reject permanente por culpa técnica de Shipro. NO SE AUTOMATIZA (decisión Nacho, modelo refinado 2026-09-10).** Al refinar el modelo, la mayoría de sub-casos se disolvieron o quedaron fuera del alcance de "cancel decision":
  - **Credencial vencida / CP fuera de cobertura**: el courier no cotiza → la opción no aparece en `/cotizar` → el envío no llega a crearse con ese courier. Pieza 5 no aplica.
  - **Dirección inválida**: pre-despacho vía auditoría Google Maps en `lib/geo/validar-direccion.ts` (5 checks: calle vacía / altura sin keyword / ZERO_RESULTS / no-street-level / CP mismatch) → RETENIDO. Nunca alcanza al courier. Pieza 5 no aplica.
  - **Dimensiones excedidas / dirección mal cargada por cliente**: culpa cliente → SÍ cobra Fee (comportamiento actual + política Nacho). No requiere Pieza 5.
  - **Timeout / conexión transitoria**: se resuelve con **REINTENTO** de despacho — obra ortogonal declarada pendiente hace tiempo como `Sub-fase 3` (`crear.ts:943`, `dispatch.ts:558-560`). Cuando entre el reintento auto, los timeouts se auto-resuelven → el envío eventualmente despacha → cobra normal. Ver "Obra siguiente" abajo.
  - **Bug técnico real de Shipro** (adapter roto, exception no prevista): raro por definición (bugs se detectan en dev/tests; en prod se fixean rápido). **NO auto-distinguible** hoy — los errores de despacho son `Error` genéricos con prefijo convencional en el mensaje (`CourierTimeout:` y `CredencialesPropiasIncompletas:` son los únicos tipados; el resto es string libre). Automatizar la clasificación requeriría refactor con `class AdapterError extends Error` + trazabilidad estructurada en `FinanzasEnvio.causaFalla` — maquinaria desproporcionada para un caso raro.
  - **Manejo actual**: cuando Nacho detecta que un bug propio causó cobros indebidos (grupo de envíos con mismo error, mensaje específico), se ajusta a mano vía `POST /api/admin/finanzas` credit + audit trail. Recurso ya disponible. Disciplina operativa, no automation.
  - Si en el futuro aparece frecuencia real (varios eventos que ameriten automatizar), reabrir como deuda propia con prerequisito `Sub-fase 3` (retry auto) — cuando haya retry, los "resistentes post-N-retries" son candidatos naturales a clasificar Shipro-technical.

**Scope global:** ALTO. Piezas 1-4: 4 commits locales (`ae9e700` + `c6beebe` + `9ac92d9` + `2e1d745`), ~9 archivos tocados en total, 2 helpers nuevos (`lib/finanzas/debito-aplicado.ts` + `lib/finanzas/credito-aplicado.ts`) + 1 helper compartido (`lib/finanzas/cancelacion-financiera.ts`). Cero migración de schema (`MovimientoFinanciero.tipo` es String libre → nuevo tipo `CREDITO_CANCELACION` sin cambio de schema). Money-crítico → verificación numérica ANTES/DESPUÉS en cada pieza + cross-check empírico contra el ledger real. Prioridad: **alta** (bugs de plata que se materializan al primer cliente real).

**Obra siguiente — REINTENTO auto de despacho (ortogonal, ya declarada pendiente):**
El caso "courier caído un rato → reintentar hasta lograrlo → cobrar" del modelo Nacho **no está implementado hoy** — todos los BLOQUEADO_PARCIAL (por CourierTimeout, adapter fail, courier sin tracking) quedan en resolución manual. TODOs en el código lo declaran pendiente: `crear.ts:943` ("Sub-fase 3 agregará reintento auto") y `dispatch.ts:558-560` ("TODO DEUDA 29 Sub-fase 3: reintento automático para tramos huérfanos. Sub-fase 3: procesar-bloqueados-parcial similar al patrón saldo/depósito"). Cuando esta obra se aborde (mini-obra propia): agregar `procesar-bloqueados-parcial.ts` con reintento con backoff. Interacción con débito unificado: los helpers P1+P4 (`debitoAplicadoEnvio`/`creditoAplicadoEnvio`) ya cubren la idempotencia de cualquier retry — el nuevo handler solo agrega el trigger, la aritmética delta protege. Con reintento + P1-P4, el circuito despacho+débito queda robusto e2e. **No bloquea el deploy de P1-P4**; se hace después como pieza ortogonal.
...
## DEUDA 180 — Mercado Envíos Flex (MEF): integración con Mercado Libre (registrada 2026-09-13, lidera Chat D, **Fase 1 núcleo RESUELTA 2026-09-21 + Fase 2 ruteo por zonas RESUELTA 2026-09-24 → ver DEUDAS-RESUELTAS.md**; pendientes conocidos explícitos: (1) shape real del payload ML (extracción defensiva; se ajusta con el primer envío real), (2) handshake del webhook real de ML (heredado de Fase 1 — sandbox impracticable; se valida con primer webhook real), (3) wiring de los 2 crons MEF (`mef-sincronizar-zonas` + `mef-procesar-notificaciones`) en el crontab del server — bucket de 5 no-wireados. **DEUDA nginx real-IP** registrada abajo como prerequisito de `ML_WEBHOOK_IP_ENFORCE=true`. Fases 3+ (enriquecimiento, tracking downstream, excepciones, etiqueta ML) las dirige Chat D como pendientes de roadmap, NO como TODOs de Chat A.)
...
- **Step 3 — rutas OAuth (install-link + callback + página de éxito) ✅ HECHO local 2026-09-14 (commits `6ea745b` + `cbc96da`).** Chat D confirmó el authorize endpoint de ML para Argentina (MLA) contra docs oficiales: `https://auth.mercadolibre.com.ar/authorization`. 4 archivos nuevos/modificados:
  - `lib/mercadolibre/tokens.ts` (modificado, aditivo): expone `ML_AUTHORIZE_URL` + `ML_OAUTH_CALLBACK_PATH` + `ML_API_BASE`. Un solo source of truth para las 3 URLs de ML. Multi-country queda documentado como follow-up (sacar `ML_AUTHORIZE_URL` a env var el día que Shipro opere MX/BR/CO).
  - `app/api/mercadolibre/install/link/route.ts` (nuevo, ~117 líneas): mirror byte-a-byte del twin Tiendanube (`app/api/tiendanube/install/link/route.ts`). Gate `admin_shipro/operador_shipro`, valida empresa, env-guard `MERCADOLIBRE_CLIENT_ID`, `randomBytes(24).toString("base64url")` + expiración 7d, `prisma.tokenVinculacionMercadoLibre.create`. URL de authorize armada con `URLSearchParams` (encoding correcto): `response_type=code&client_id=<env>&redirect_uri=<APP_URL>/api/mercadolibre/oauth/callback&state=<token>&scope=offline_access+read+write`. Devuelve `{ url, expira }`.
  - `app/api/mercadolibre/oauth/callback/route.ts` (nuevo, ~252 líneas): mirror simplificado del twin Tiendanube (sin registro de carrier ni webhooks — esos son Fase 3+). Validar state (existe + no expira + `usadoEn=null`) → `exchangeCodeForToken(code, redirectUri)` con retorno 502 sin quemar token si falla → best-effort `GET /users/me` para nickname (nunca rompe la instalación si falla) → **cross-install guard** por `mlUserId` (findUnique `CuentaMercadoLibre` → si `empresaId` distinta: 409 + `AuditoriaConfiguracion.create({campo:"mercadolibre:cross_install"})` + `console.error` + **NO** cambia nada + **NO** quema token) → **$transaction atómico** [`cuentaMercadoLibre.upsert` con `accessToken`+`refreshToken` **ENCRIPTADOS** via `encryptSecret` (AES-256-GCM), `tokenExpiraEn = now + expiresIn*1000`, `scope`, `nickname`, `estado="activa"`, `desvinculadaEn=null`; `tokenVinculacionMercadoLibre.update({usadoEn: now})`] → best-effort `Conexion.upsert({plataforma:"MERCADOLIBRE", mecanismo:"OAUTH", estado:"ACTIVA", referenciaExterna: String(mlUserId)})` → redirect a `/mercadolibre/instalado?mlUserId=<id>&nickname=<n>`.
  - `app/mercadolibre/instalado/page.tsx` (nuevo, +101 líneas): mirror del twin `app/tiendanube/instalado/page.tsx`. Public page (top-level, sin session), Suspense + client component, wordmark Shipro + icono verde + copy honesto para Fase 1 ("Cuenta de Mercado Libre conectada; te avisamos cuando terminemos de activar cotización + etiquetas"). Query params leídos: `mlUserId` + `nickname`. Match verificado con la redirect target del callback — cero 404.
  - **Disciplina de burn del token**: `usadoEn=now` se setea SOLO dentro del `$transaction` del happy path. Exchange failure (502) y cross-install (409) retornan antes → token queda vivo para reintento. Mirror byte-a-byte del twin.
  - **Env vars requeridos (local + prod gated)**: `MERCADOLIBRE_CLIENT_ID` + `MERCADOLIBRE_CLIENT_SECRET`. Sin ellos, install-link tira 500 claro y el callback falla en el intercambio con 502.
  - tsc 0. Cero cambios en Envio/cotizador/crear/dispatch/schema/otras rutas.

- **Step 4 — webhook receiver ML ✅ HECHO local 2026-09-14 (commit `cf49e93`).** 3 archivos (2 nuevos + 1 línea aditiva en `proxy.ts`, +276 líneas net):
  - `lib/mercadolibre/webhook-verify.ts` (nuevo, ~110 líneas): `verifyMercadoLibreWebhookSignature({payload, xSignature, xRequestId})`. Parsea el header `x-signature` con formato `ts=<millis>,v1=<hex>`, arma el manifest **exacto** `id:${resourceId};request-id:${xRequestId};ts:${ts};` donde `resourceId = payload.data.id` (⚠️ NESTED — el id del RECURSO, ej. shipmentId; NO `body.id` top-level, que es el notification id / dedup key). HMAC-SHA256 con `MERCADOLIBRE_CLIENT_SECRET` (fail-fast local getter) → `timingSafeEqual` constant-time contra `v1`. Guard `/^[0-9a-f]{64}$/` sobre v1 (rechazo silencioso con `false` en headers basura, sin crashear). Missing headers / data.id → false.
  - `app/api/mercadolibre/webhooks/route.ts` (nuevo, ~155 líneas): `POST` runtime nodejs. Path `/api/mercadolibre/webhooks`. Read raw → JSON.parse → verify firma → extract fields (`notificacionMlId = body.id`, `topic`, `resource`, `mlUserId`, `attempts`, `sent`, `received`) → parse `shipmentId` del resource si topic="shipments" via regex `\/shipments\/(\d+)` → resolver `empresaId` via `prisma.cuentaMercadoLibre.findUnique({where:{mlUserId}})` (unique) → **persist idempotente**: `prisma.notificacionFlex.create({data:{notificacionId: notificacionMlId, ...}})` con `try/catch (e.code === "P2002")` → retry del mismo `body.id` = P2002 = 200 idempotente `{duplicated: true}` sin throw ni duplicado. **Unknown seller**: si no hay match en `CuentaMercadoLibre`, persistir con `empresaId=null` + `estado="huerfana"` + 200 (no queremos que ML retry a un seller que no gestionamos; el worker de Fase 3 podrá re-vincular si aparece el OAuth después). **Staging only**: cero envío create, cero `crearEnvio`, cero `movimientoFinanciero` — grep confirmado. Fase 3 (worker separado) consume `NotificacionFlex` async con `mlFetch` para enrichment.
  - `proxy.ts` (+1 línea aditiva): agrega `"/api/mercadolibre/webhooks"` a `PUBLIC_API_EXACT`. Auto-auth via HMAC firma (self-auth); ML llega sin sesión Shipro. Decisión de scope: se usó `PUBLIC_API_EXACT` en vez de `PUBLIC_API_PREFIXES` porque la ruta base `webhooks/route.ts` resuelve a `/api/mercadolibre/webhooks` (sin trailing slash) — un prefix con slash no matchearía, y sin slash sería permisivo. EXACT es 1 línea, precisa; futuros webhook routes se pueden migrar a prefix si aparecen múltiples.
  - **Prevención del bug #1**: `resourceId` (para el manifest) vs `notificacionMlId` (para el dedup) están **nombrados sin ambigüedad + comentados con ⚠️ en el código**. Confundirlos → firmas fallan silenciosas (401 permanente + ML retry loop) o duplicados en la tabla. Nombres distintos + rutas distintas de lectura (`payload.data.id` vs `payload.id`) hacen el bug imposible por construcción.

- **Fix latente cerrado — `/api/mercadolibre/oauth/callback` a rutas públicas ✅ HECHO local 2026-09-14 (commit `e0634e7`, +1 línea aditiva en `proxy.ts`).** Encontrado por Claude Code durante el recon del step 4: el callback OAuth de step 3 no estaba en `PUBLIC_API_EXACT`, así que en producción el proxy lo hubiese clasificado como "session" y respondido 401 antes de que el handler corra (el seller llega redirigido por ML sin sesión Shipro). Sin este fix, todo el flow OAuth de MEF estaría roto en prod. La línea agregada mirror del twin Tiendanube (L19): `"/api/mercadolibre/oauth/callback" // Callback OAuth de Mercado Libre — self-auth via token de vinculación en state (DEUDA 180 MEF Fase 1 step 3).` El fix no cambia comportamiento en local (dev proxy no enforced), pero cierra el gap para prod.

**⚠️ Follow-ups abiertos de Fase 1 (no bloqueantes)**:
1. **`enviarMailAlertaCruceMercadoLibre` helper en `lib/mailer.ts`** (mirror del twin `enviarMailAlertaCruceTiendanube` L667). Hoy el cross-install guard hace `AuditoriaConfiguracion.create` + `console.error` — el mail es visibilidad extra al equipo Shipro. Caso raro; el audit ya lo captura. Aditivo (~40 líneas). TODO documentado en el callback.
2. **Env vars `MERCADOLIBRE_CLIENT_ID` + `MERCADOLIBRE_CLIENT_SECRET`** — setear en `.env.local` para probar OAuth + webhook end-to-end en dev; prod queda gated hasta que Nacho + Chat D lo abran.
3. **Enrichment del webhook (GET `/shipments/{id}` via `mlFetch`)** — diferido explícitamente a Fase 3 (worker separado que consume `NotificacionFlex`). Step 4 es staging puro.

**Steps siguientes**:
- **Step 5 (adapter Mercado Envíos Flex)** — **Chat B territory** (`lib/couriers/*`; Chat A no toca).
- **Fase 2 (worker que procesa `NotificacionFlex` → crear envíos → mover state)** — coordina Chat D (probable Chat A por implementación en `app/api/cron/*` o similar; a definir con Chat D).
- **Fase 3-4** siguen fuera del alcance actual — coordinación con Chat D + Chat B cuando llegue el turno.

**Estado global**: **Fase 1 (núcleo, Chat A) DEPLOYADA + VERIFICADA EN PROD 2026-09-15** (deploy gated, corrido por Nacho). Prod pasó de `2279701` a `75d287e` — el Paso 0 pre-deploy confirmó que el pending era exactamente Consola A+B safe (ya deployado antes por su cuenta) + MEF Fase 1 + docs, sin nada money-crítico mezclado.

- **Migración aplicada**: `20260913235719_mef_modelo_datos` (41 migraciones total en prod). SQL inspeccionado byte-a-byte antes del `prisma migrate deploy`: 3 `CREATE TABLE` (`CuentaMercadoLibre`, `TokenVinculacionMercadoLibre`, `NotificacionFlex`) + 4 unique indexes + 5 índices comunes + 3 `ADD CONSTRAINT` FK a `Empresa` (CASCADE × 2, SET NULL × 1 para NotificacionFlex.empresaId). **Aditivo puro** — cero `DROP`, cero destructive `ALTER` en tablas existentes. Los 3 matches de `ON DELETE` en el grep eran cláusulas de referential integrity de las FKs nuevas, no ops destructivas.
- **Código en prod**: `lib/mercadolibre/tokens.ts` + `client.ts` + `webhook-verify.ts`; rutas `install/link`, `oauth/callback`, `webhooks`; página `/mercadolibre/instalado`; fix `proxy.ts` (callback OAuth agregado a `PUBLIC_API_EXACT`). Clean rebuild (schema.prisma + proxy.ts tocados → `rm -rf .next && npm run build`). `pm2 restart shipro #84 --update-env`.
- **Env vars cargadas en prod**: `MERCADOLIBRE_CLIENT_ID` + `MERCADOLIBRE_CLIENT_SECRET` en `/var/www/shipro/.env` (mismo lugar que `SECRET_ENCRYPTION_KEY` + `CRON_SECRET`). App arrancó sin error de credenciales ML — confirma que los getters `getMercadoLibreClientId/Secret` los ven correctamente.
- **Verificación e2e en prod**: las 3 tablas MEF existen + vacías (0 filas c/u); build incluye las 4 rutas ML; app responde. Fase 1 (OAuth + token persistence + webhook staging) **operativa en prod**.

**Botón autoservicio "Conectar Mercado Libre" ✅ HECHO + VERIFICADO local 2026-09-15 (commit `e8021c8`, +417/-45 líneas, 6 archivos).** El cliente logueado (rol `gerente_cliente` o `operador_cliente`) conecta SU PROPIA cuenta ML desde una nueva sub-page `/configuracion/conexiones` — **primer flujo self-service del sistema** (Tiendanube sigue siendo operator-triggered; ML es el debut de este patrón). Piezas:

- **Helper compartido** `lib/mercadolibre/install-link.ts` (nuevo, ~85 líneas): `crearInstallLinkMercadoLibre(empresaId)` puro — env guards + `randomBytes(24).toString("base64url")` + `TokenVinculacionMercadoLibre.create` + URL de authorize armada con URLSearchParams (`scope: "offline_access read write"`). Sin auth logic — recibe `empresaId` ya validado por el caller.
- **Endpoint operator** `app/api/mercadolibre/install/link/route.ts` (refactor cerrado, -20/+5 net): mantiene gate `x-rol admin_shipro/operador_shipro` + `body.empresaId` byte-idéntico; solo el core (token + URL) delega al helper. Comportamiento externamente observable **intacto**.
- **Endpoint session-scoped** `POST /api/empresa/mercadolibre/connect/route.ts` (nuevo, ~68 líneas): `getToken({req, secret: NEXTAUTH_SECRET})` → 401 si null, 400 si `token.empresaId === null` (usuarios Shipro), 403 si rol no en `["gerente_cliente", "operador_cliente"]`, y **`empresaId = token.empresaId as number`** — SOLO del JWT firmado. 🚨 **SECURITY INVARIANT**: el body NUNCA se parsea (grep de `body.empresaId | request.json | req.json` → cero matches en código, solo en comentarios explicativos). Un cliente malicioso que envíe `body: { empresaId: <otra> }` es ignorado por construcción.
- **Endpoint session-scoped** `GET /api/empresa/mercadolibre/conexion/route.ts` (nuevo, ~65 líneas): read del connected state via `prisma.cuentaMercadoLibre.findUnique({where:{empresaId: token.empresaId}, select:{mlUserId, nickname, estado, tokenExpiraEn, vinculadaEn}})`. Devuelve `{conectada: true, mlUserId, nickname, estado, tokenExpiraEn, vinculadaEn}` si `estado="activa"`; `{conectada: false, estado?}` en cualquier otro caso.
- **Page cliente** `app/(dashboard)/configuracion/conexiones/page.tsx` (nuevo, ~180 líneas): mount fetch al GET conexion; card ML con status pill; si `conectada:false` muestra botón "Conectar Mercado Libre" que hace POST connect → `window.location.href = data.url` (redirect al authorize de ML); si `conectada:true` muestra card verde con `@nickname` (o `#mlUserId` si nickname null). Styling mirror de `/configuracion/depositos` (lucide icons, `#233b6b` brand color, mismos rounded cards).
- **Sidebar tab "Conexiones"** en `app/(dashboard)/configuracion/layout.tsx` (+1 línea aditiva): tab nuevo con icon `Plug`, `visible: true` (accesible a gerente_cliente + operador_cliente + roles Shipro por igual). Sidebar preservado byte-idéntico salvo el tab nuevo.
...
- **Prueba A — webhook receiver: RESULTADO PARCIAL + HALLAZGO CRÍTICO 2026-09-21.** Objetivo original: validar que el receiver acepta un webhook REAL de ML (handshake firma real). Estado: **validación self-signed OK (2026-09-18)** — script `test-webhook-selfsigned.mjs` firmó un POST a `/api/mercadolibre/webhooks` con manifest + HMAC exactos del receiver (usando el mismo `MERCADOLIBRE_CLIENT_SECRET` de prod, secret nunca expuesto). Resultados en `NotificacionFlex`: positivo 200 + fila con `estado="recibida"` + `mlUserId=3686169320` (BigInt OK) + `empresaId=1` resuelto → confirma el fix BigInt en el READ path; negativo con `v1=0*64` → HTTP 401 sin persistir (verify-before-persist en `webhooks/route.ts:58-60`). **PERO**: el self-signed validaba nuestro-firmante-vs-nuestro-verificador — un malentendido del manifest se autovalidaría. El handshake con ML REAL era el pendiente único hasta 2026-09-21, cuando se descubrió que:

- **🚨 HALLAZGO DE SEGURIDAD (2026-09-21, crítico) — el receiver rechazaría los webhooks reales de ML.** Research propia + GN + doc oficial de seguridad ML vía Chat D confirman: **el marketplace de ML (topics: `shipments`, `questions`, `items`, `flex-handshakes`, etc.) NO firma sus webhooks**. Autentica por: (a) IP DE ORIGEN (lista publicada de ~14 IPs que **cambian sin aviso**), (b) HTTPS con cert de ML, y (c) validación server-side vía GET autenticado del recurso mencionado. El header `x-signature`/HMAC es de **Mercado PAGO** (topic `payments`), no del marketplace. Nuestro receiver valida SOLO por `x-signature` → **rechazaría con 401 los webhooks reales de ML** → notificaciones de envíos con un cliente real **se perderían**. La insistencia de Nacho en el e2e real destapó este bug ANTES de producción operativa (cero clientes ML afectados). El self-signed de la Prueba A no lo detectó por diseño (ambos lados los firmábamos nosotros).

- **Prueba A e2e con ML real — CONCLUIDA SIN CAPTURA (agotados los caminos del sandbox)**: 
  - Endpoint de captura `/api/mercadolibre/webhookscapture` desplegado + verificado accesible (POST → 200, público, HTTPS OK desde afuera). Sin guiones en el path — el panel DevCenter de ML rechaza guiones en URLs de notificación (dato descubierto en el intento: `webhooks-capture` → "URL inválida"; `webhookscapture` → aceptada).
  - Guardar la URL en DevCenter NO dispara ping de verificación (la doc de 2015 que sugería un ping en el `save` no aplica al panel actual).
  - Publicación de prueba en la cuenta ML sigue pausada (PolicyAgent persistente); sin item publicado no se puede preguntar/comprar → cero eventos gatillados.
  - `GET /missed_feeds` sobre el test seller devuelve `{"messages":null}` → ML nunca intentó mandar ninguna notificación (nada que reenviar).
  - Nueva fuente sugerida para GN: `https://developers.mercadoenvios.com/general/testing` (testing de Mercado Envíos — a investigar por si aporta un camino para gatillar webhooks reales sin comprar).
  - Conclusión: sin cliente ML real, no hay forma de forzar el handshake real desde el sandbox. Se validará con el PRIMER CLIENTE REAL — mientras tanto, arreglamos el receiver para que acepte lo que ML manda de verdad.

- **Egress server→ML CONFIRMADO OK (2026-09-21)**: `curl` autenticado `GET /users/me` desde `/var/www/shipro` → HTTP 200. El `000` anterior fue transitorio (reconexión Linode). **Consecuencia**: el candado del GET autenticado del shipment (2do lock del arreglo) es **viable en prod** sin cambios de infra.

- **Diseño del arreglo del receiver (PENDIENTE de construir con cabeza fresca + decisión de política de Chat D)**:
  1. **NO exigir `x-signature`**: sacar el 401-por-firma-ausente (`webhooks/route.ts:58-60`). Si `x-signature` viene, verificar como bonus + log; si no viene, seguir sin rechazar.
  2. **IP allowlist como defensa fail-open-but-loud**: comparar la IP de origen contra lista en env `MERCADOLIBRE_WEBHOOK_IPS` (comma-separated, ~14 IPs que ML publica y **cambian sin aviso**). ⚠️ El proxy NO enriquece paths públicos con `x-ip-origen` (ver `proxy.ts:111`) — el receiver debe leer `x-forwarded-for` directo, primer entry (mirror del patrón `proxy.ts:148-151`). Flag `MERCADOLIBRE_WEBHOOK_IP_ENFORCE` (default `false` = procesa igual + log; setear a `true` cuando la allowlist esté probada estable).
  3. **CANDADO REAL (2 locks superpuestos)**:
     - **Lock A — seller resolution**: resolver `user_id` → `CuentaMercadoLibre.findUnique({where:{mlUserId: BigInt(x)}})`. Solo procede lo que resuelve a un seller conectado (ya funciona hoy — se mantiene).
     - **Lock B — GET autenticado**: `mlFetch(empresaId, "/shipments/{id}")` con el token del seller. Solo procede lo que resuelve a un shipment real en la cuenta ML. Un atacante que arme un JSON con `user_id=<real>` + `shipment_id=<inventado>` cae por el 404 del GET. Egress OK → viable.
  4. **Mantener** dedup por `notificacionId @unique` + persistencia `NotificacionFlex` idempotente (create + catch P2002) — funcionan bien, no tocar.
...
**Avance MEF Fase 2.3.a (resolver puro CP→zona→courier) ✅ HECHO local 2026-09-22** (commit `e6c732b`) + **tweak Chat D 2026-09-23** (variantes `sin_zonas_flex` y `anomalo` fail-fast). Helper puro en `lib/mercadolibre/resolver-courier-zona.ts`: dado `(empresaId, cp)` devuelve `{ok:true, courierId, ...}` o variantes de bloqueo (`input_invalido`/`sin_cuenta_ml`/`sin_zonas_flex`/`cp_no_matchea`/`zona_sin_courier`/`anomalo`). Filtro `enabled=true` DENTRO del where; normalización simétrica con el sync 2.1 (`String(cp).trim()`); fail-fast si aparecen >1 zonas activas (GN garantiza disjuntos entre activas — si aparecen, algo anda mal, NO ruteamos silenciosa). Cero side-effects, cero persistencia, cero money.

**Avance MEF Fase 2.3.c (worker que crea el envío desde NotificacionFlex valido) ✅ HECHO local 2026-09-23** (Chat A, pedido por Chat D). MONEY-CRITICAL — aditivo puro; motor de precio INTACTO. Sin tocar `cotizador.ts`; `crear.ts` recibe SOLO plumbing aditivo de vínculo ML (mirror byte-a-byte de los fields Tiendanube). Piezas:
- **Migración `20260923183958_mef_envio_ml_link`** (aditiva pura): 2 ADD COLUMN nullable en `Envio` (`mercadolibreShipmentId TEXT`, `mercadolibreOrderId TEXT`) + 1 CREATE INDEX (`mercadolibreShipmentId`). Cero DROP.
- **Reconciliación con `OrdenExterna.mercadolibreShipmentId` (pre-existente sin uso)**: se eligen **campos DIRECTOS en `Envio`** siguiendo el precedente Tiendanube (`tiendanubeFulfillmentOrderId`, `tiendanubeStoreId`, `tiendanubeOrderId`). Documentado como comentario en el schema; `OrdenExterna` queda como legacy scaffold sin consumir. Usar `OrdenExterna` rompería la simetría con el twin.
- **`lib/mercadolibre/crear-envio-flex.ts`** (nuevo, ~430 líneas): `procesarNotificacionFlex(id)` + `procesarNotificacionesFlexPendientes(limit)`. Molde Tiendanube (labels/generate): guard idempotente por `Envio.mercadolibreShipmentId` → carga `ShipmentFlex.payloadRaw` → llama `resolverCourierPorCpFlex` → despacha por variante. **Ninguna intervención de precio**: `crearEnvio(input)` con `idempotencyKey="mef-${shipmentId}"` + `permitirBloqueoPorDeposito=true`; el motor cobra según rama del courier (`CredencialCourier(empresaId, nombreCourier).usaCredencialesPropias`) — Rama A cascada completa, Rama B Fee-only — como cualquier envío. Cero flag canal-aware, cero SMO exento.
- **Variantes de bloqueo dedicadas** (nuevos estados literales en `lib/utils/estados.ts` catálogo `ESTADOS_BLOQUEO_FLEX`, cross-ref [[DEUDA 173]]): `flex_fuera_cobertura` (cp_no_matchea), `flex_zona_sin_courier` (zona_sin_courier), `flex_sin_config` (sin_zonas_flex), `flex_cp_no_extraido` (cpDestino=null en ShipmentFlex), `flex_anomalo` (resolver anomalo — fail-fast por drift), `flex_datos_incompletos` (payload sin destinatario/peso). Los envíos BLOQUEADO_flex_* nacen con SHP-BLOQ-* tracking + no ejecutan débito (no entran a la sección de motor de precio de `crear.ts`) — visibilidad al vendedor sin cargo.
- **`app/api/cron/mef-procesar-notificaciones/route.ts`** (nuevo, mirror byte-a-byte de `mef-sincronizar-zonas`): handler mínimo, `dynamic = "force-dynamic"`, delega a `procesarNotificacionesFlexPendientes`. Auth via `proxy.ts` kind="cron" (`Authorization: Bearer $CRON_SECRET`).
- **Idempotencia — 2 CAPAS**:
  1. Guard early-out por `Envio.mercadolibreShipmentId` con índice dedicado (@@index) — evita entrar al motor si ya existe.
  2. `crearEnvio` recibe `idempotencyKey="mef-${shipmentId}"` + `@@unique([empresaId, idempotencyKey])` como red final anti-race.
- **`crear.ts` — SOLO plumbing aditivo** (3 hunks, +12 líneas net):
  1. Interface `CrearEnvioInput`: 2 fields opcionales (`mercadolibreShipmentId?`, `mercadolibreOrderId?`).
  2. Destructure del input: agrega los 2 nombres.
  3. Persist en `tx.envio.create.data`: 2 fields con `?? null`.
  Grep del diff busca `rama|smoNeto|montoDebito|resolverSmoNeto|aplicarMarkup|cotizar\(|usaCredencialesPropias` → **1 solo match** = el comment de seguridad `// ⚠️ MONEY-SAFE: SOLO plumbing. NO tocan rama/SMO/montoDebito/cotización.` — CERO líneas de código tocan pricing.
- **`cotizador.ts` UNTOUCHED**: `git diff --name-only lib/cotizador.ts` → vacío. Verificable byte-a-byte.
- tsc 0.

**🔒 NOTA — "SMO exento en Flex" — premisa REVISADA/descartada (2026-09-23)**: En el recon inicial de Fase 2.3.c se reportó que Flex necesitaría exentar SMO (implicando cambio al motor de precio). Nacho + Chat D revisaron y **descartaron la premisa**: Flex es un canal más y cobra según la rama del courier asignado por el vendedor en Fase 2.2, exactamente como cualquier envío Tiendanube/dashboard/plugin con el mismo courier. El motor de precio (`lib/cotizador.ts` + `aplicarMarkup` + `resolverSmoNeto` + `calcularFeeOperacion`) **NO se toca**. La rama la decide `CredencialCourier.usaCredencialesPropias`; cada variable (SMO, markup Shipro, markup fijo, intermediario, Fee) tiene su propio gate per-courier o per-empresa que ya existe. **Si en el futuro** el negocio quisiera exentar SMO específicamente para Flex (con data de siniestralidad que lo justifique), sería una pieza NUEVA canal-aware — probablemente 1 flag `smoExento?: boolean` en `CotizarInput` + campo `canalOrigen` en `Envio` — pero ES un cambio de motor y se diseña como pieza separada, con gran cuidado. **NO es pendiente activo**. Registrado acá para no perderlo.

**Pendiente Fase 2 — el ruteo cierra Fase 2 con 2.3.c EN LOCAL; queda por deploy gated Nacho + Chat D**. Piezas ya construidas (Fase 2.1 EN PROD, 2.2 EN PROD, 2.3.a + 2.3.c local). Piezas remanentes en el roadmap de Chat D:
- Fase 3 (worker consumidor de notificaciones reintentables — get_fallido_reintentable / get_shipment_no_existe).
- Fase 4 (state machine downstream: los shipments creados van cambiando estado según los webhooks siguientes; usa `ShipmentFlex.payloadRaw` + `estadoShipment` que el receiver retrofit ya persiste).
- Fase 5 (UI hub + Chat B territory — adapters ML si aplica).
