...
- **Prueba A — webhook receiver ✅ VALIDADA 2026-09-18 (vía webhook AUTO-FIRMADO, Camino 2).** Decisión de Chat D tras descartar el path del sandbox de ML (PolicyAgent persistente + ~10 obstáculos de operativa ML — ninguno de código nuestro; el sandbox no fue viable como banco de pruebas). Método: script throwaway `scripts/test-webhook-selfsigned.mjs` que firmó un POST a `/api/mercadolibre/webhooks` con el manifest + HMAC EXACTOS del receiver (leídos byte-a-byte de `lib/mercadolibre/webhook-verify.ts`, mismo `MERCADOLIBRE_CLIENT_SECRET` de prod), ejecutado en el server (sin exponer el secret — nunca se imprimió). Resultados verificados en `NotificacionFlex` de prod:
  - **POSITIVO** (firma válida): HTTP 200; fila creada con `estado="recibida"`, `mlUserId=3686169320` (BigInt OK, no INT4 overflow), `empresaId=1` **RESUELTO** (no null) — **valida el fix BigInt en el READ path** (Chat D's 3rd-bug flag: el lookup `where:{mlUserId: BigInt(...)}` matchea la cuenta real), `topic="shipments"`, `shipmentId` extraído del resource por el regex, `payloadRaw` completo persistido. ✅
  - **NEGATIVO** (v1 tampered = `"0".repeat(64)`): HTTP 401 `"Invalid signature"`, **CERO filas** creadas en `NotificacionFlex`. El receiver rechaza firmas inválidas ANTES de persist (verify-before-persist en `webhooks/route.ts:58-60`). ✅
  - **Qué valida**: (a) construcción exacta del manifest `id:${resourceId};request-id:${xRequestId};ts:${ts};` con `resourceId=data.id` NESTED, (b) HMAC-SHA256 constant-time con `timingSafeEqual`, (c) resolución `empresaId` via `CuentaMercadoLibre.findUnique({where:{mlUserId: BigInt(x)}})` — el fix BigInt del READ path funciona con data real, (d) persistencia idempotente por `notificacionId @unique` en estado="recibida" (no "huerfana"), (e) extracción `shipmentId` del resource, (f) rechazo del receiver ante firma inválida sin tocar la tabla.
- **PENDIENTE ÚNICO (registrado explícito como DEUDA 133): handshake con firma REAL de ML.** El self-signed valida que NUESTRO firmante = NUESTRO verificador — un malentendido del manifest se autovalidaría porque ambos lados los firmamos nosotros. Falta que **ML firme un webhook real** contra el mismo manifest → confirma el shared secret de ambos lados matchea + el protocolo real es el que asumimos. Se validará automáticamente en el **primer webhook real de venta Flex** (una compra real de un buyer a un item publicado por el test seller, o cualquier evento genuino que ML dispare). Hasta ese hito, **NO dar Fase 1 100% cerrada**.
- **Cleanup completo** (2026-09-18): script throwaway `scripts/test-webhook-selfsigned.mjs` borrado del server + del repo (git rm + commit + push + pull en el server); endpoint dev-only `app/api/dev/ml-token/route.ts` borrado del repo + confirmado ausente del build de prod (el 401 residual en `/api/dev/ml-token` es del proxy interceptando `/api/dev/*` sin sesión, no del endpoint — la ruta ya no existe). Queda el item de test ML `MLA2104360157` publicado (inofensivo, lo maneja Chat D en su turno).
