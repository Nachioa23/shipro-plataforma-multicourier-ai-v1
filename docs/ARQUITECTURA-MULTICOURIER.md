...
49. **Modelo de First-Mile rectificado (2026-05-19).** Despues de analizar el ejemplo operativo de 3 couriers (Mocis consolidador + Andreani + un tercer courier con coberturas mixtas), se descarta el modelo de "modoFirstMile como atributo configurado por el cliente". Nueva descomposicion:
    ...
    **Cascada inteligente al PUT del deposito (implementada en commit 3add6cc, 2026-05-20):** cuando el cliente cambia `Deposito.courierRecolectorId` via PUT al endpoint del deposito, el backend ejecuta una cascada atomica:
    ...
    Validacion best-effort de cobertura del CP del deposito por el consolidador: si el consolidador no tiene filas en `SucursalCourierCp` (caso Mocis, modelado con sucursal unica via `cpDepositoConsolidador`), SKIP la validacion y reporta el motivo en `skipsDeValidacion`.

    [ACTUALIZACION - 2026-06-01]: DEUDA 32 RESUELTA en Fases A-K (commit 10eda29 cierra la unificacion completa). La cobertura de couriers se maneja ahora via servicios del modelo ServicioCourier (Fase A-C), sync periodica de sucursales por cron (Fase G), y el registro RegistroCoberturaVacia que loguea cotizaciones vacias para auditoria de la red (Fase J). El flag tieneSucursales del modelo Courier fue eliminado (Fase K) y se deriva del servicio entrega_sucursal.
...
model Courier {
  ...
  // [ACTUALIZACION - 2026-06-01]: campo tieneSucursales ELIMINADO en Fase K
  // de DEUDA 32+37 (commit 10eda29). Se deriva ahora del servicio
  // entrega_sucursal via helper tieneSucursales(courier) en
  // lib/couriers/serviciosSoportados.ts. Una sola fuente de verdad.
...
| DEUDA 32 | Sincronizacion periodica de cobertura de couriers en background | RESUELTA (Fases A-K) | RESUELTA 2026-06-01 (commit 10eda29 cierra el proyecto fusionado DEUDA 32+37). Implementado en 11 fases (commits e829716, bcb4ff9, ae9b724, 6615e84, 21c650b, ef5e1c6, 73c805e, 0322ecc, d3e9a17, 10eda29): (A-C) modelo de datos de servicios de courier + registry centralizado + seed con 8 servicios y 2 capacidades por courier; (D-F) factory expuesto + endpoint admin + funcion sync generalizada para cualquier courier con `tieneSucursales=true`; (G) endpoint cron `/api/cron/sincronizar-couriers` con auth `CRON_SECRET` + endpoint manual `/api/admin/couriers/[id]/sincronizar` para sync on-demand; (H) drawer admin de couriers con 4 secciones (Info, Consolidador, Servicios, Sincronizacion) + Modelo C de defensa en profundidad; (I) wizard de alta de courier (drawer lateral) con estado adaptativo segun integrables; (J) deteccion de cobertura vacia + banner ambar UX + tabla auditable RegistroCoberturaVacia para analitica de la red logistica; (K) eliminacion del campo legacy `tieneSucursales` del modelo Courier + helper derivado en `lib/couriers/serviciosSoportados.ts` (una sola fuente de verdad). Detectada 2026-05-19 durante rectificacion de 6.D. |
...
| DEUDA 35 | Los endpoints de creacion de envio no mapean tipoOrigen del body al input de crearEnvio | RESUELTA (post-DEUDA-34) | RESUELTA 2026-05-27. Implementado en 3 archivos: (1) app/api/envios/route.ts y app/api/envios/manual/route.ts ahora mapean body.tipoOrigen al input de crearEnvio CON validacion (rechazan strings invalidos con HTTP 400; valores aceptados: "recoleccion_courier" o "drop_off_cliente"; si el body no manda nada se respeta el default existente). (2) lib/envios/crear.ts ahora persiste tipoOrigen en el data block del tx.envio.create — bug pre-existente destapado por TEST 2: dispatch ramificaba correctamente por caso A pero el campo en la fila Envio siempre quedaba con el default del schema. Tests E2E: 3/3 PASS (default sin campo, drop_off_cliente activa caso A, basura rechazada con 400). El caso A de dispatch.ts se puede activar via API por primera vez. Detectada 2026-05-21 durante TEST 5 de Sub-fase 6.D.5 (commit ad68902). |
...
| DEUDA 37 | ABM unificado de administracion de couriers (espacio Shipro y espacio cliente) | PARCIALMENTE RESUELTA (Fases H-I) — Espacio Shipro completo, Espacio Cliente pendiente | RESUELTO PARCIALMENTE 2026-06-01 (commits 6615e84, 21c650b, ef5e1c6, 73c805e, 0322ecc — fusionada con DEUDA 32 como proyecto unico). ESPACIO SHIPRO entregado en Fases H (drawer admin de couriers con 4 secciones: Info General, Consolidador con AutocompleteAddress, Servicios con 8 switches + 3 estados visuales + candado de Modelo C, Sincronizacion con stats adaptativos) y Fase I (wizard de alta de courier — drawer lateral con estado adaptativo segun integrables disponibles). El admin Shipro puede editar todos los aspectos del courier desde un solo lugar. ESPACIO CLIENTE PENDIENTE (sigue como Post-MVP): el ABM del lado cliente (activar/desactivar courier, elegir si es recolector, agregar credenciales propias, subsidiar costo publicado) sigue parcialmente en TransportesTab; no se construyo un ABM dedicado. Vision del director (2026-05-26): el ABM completo requiere construir el espacio cliente unificado, que se posterga porque no es bloqueante hoy (TransportesTab cubre lo minimo operativo). Registrada durante Sub-fase 6.D.7. |
