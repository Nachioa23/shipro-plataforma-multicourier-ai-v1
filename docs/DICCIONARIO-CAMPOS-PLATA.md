...
>
> **Estado:** foto de solo-lectura tomada en el commit `ebe88df` (2026-08-28). Ningún campo fue renombrado todavía — este documento es el paso previo a decidir qué corregir.
...
|---|---------------------|-----------|----------|
| 1 | **Cotización virtual full al cliente** (lo que Shipro cotiza al crear el envío, con peso/dims declarados) | `precioFactura` (congelado al alta) | ✅ |
| 2 | **Facturado real full al cliente** (post-liquidación, con el costo real del courier) | **NO existe como campo** — es la suma `precioFactura + costoAforo`, calculada al liquidar | ⚠️ derivado |
...
> **Nota crítica sobre `precioFactura`:** NO se actualiza al importar la liquidación del courier. Queda **congelado en la cotización del alta**. El desvío por aforo se guarda aparte en `costoAforo`, y el total real al cliente es la **suma de los dos** (`precioFactura + costoAforo`), calculada en la liquidación mensual. Esta es una decisión de diseño deliberada (documentada en `conciliacion/route.ts` L145-152), no un bug.
...
| `precioFactura` | "Lo que Shipro le debita a la empresa" | Cotización virtual **congelada al alta**. NO es el facturado real. El real = `precioFactura + costoAforo`. | ⚠️ (interpretación) | 2 |
...
5. `precioFactura` → aclarar "congelado al alta; el real es + costoAforo".
...
13. `precioProveedor` → aclarar que es forma nativa (mezcla IVA cross-courier). *(cruza con DEUDA 152)*

### Grupo 3 — Campos FANTASMA (limpiar aparte)
14. `porcentajePrecioFactura` — sin usos.
15. `quiereSeguroCourier` — pipeline no lo lee.
16. `seguroFijoIntermediarioConIva` — ghost + nombre contradice al seed.
...
- **FASE 3 — Grupo 1 (renames por rol)** — ⏳ pendiente. Renombrar campos mal nombrados según su rol real (empezando por `precioFactura → tarifaFullCotizada`). Registrada como **[[DEUDA 158]]**. Se cruza con [[DEUDA 154]]/[[DEUDA 157]] (rediseño markup).
